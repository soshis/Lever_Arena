...
235@definiton: Association nose spoke and lever push: Begin with 10 nose spokes-only with a lever at an unreachable (low) position 
...
with open("constant.txt") as constant:
    for line in constant:
        if line[3:5] == animal_number:
            threshold_error=np.ones(10)*(int(line[7:12])-100)
            threshold = np.linspace(int(line[7:12]),3200,10)
...
lever_obj.set_threshold_error(threshold_error)
 ...
 # Look at a peak in the signal (a local maximum) until a max it reached, it computes if it was a Good trials or not a good trial
    if leverL > leverL_old :
        ...
        if it_thres == inc_thr:
            ...
            it_thres = 0           
    elif leverL < leverL_old  and nose < 100:    
        # Detect some fails
    ...
    if errors >= numb_error:
        ind_t = np.where(leverL_old>threshold)[-1]
        if  ind_t.size ==0 :
            ind_t = np.array([0])
